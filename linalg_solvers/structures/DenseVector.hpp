...
class DenseVector {
   ...
   public:
    ...
    /// @brief read vector from a file.
    /// @param path path to the directory containing a system of linear eqn.
    ...
    double dot_product(const DenseVector &v) const;
};
...
inline DenseVector operator*(double lhs, const DenseVector &rhs) { return rhs * lhs; }
